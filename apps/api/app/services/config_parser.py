...
def serialize_config_tree(path: str, tree: ConfigNode) -> str:
    ...
    if parser.name == "xml":
        raise GitHubClientError(
            "XML writeback is disabled until safe round-trip serialization is implemented.",
            status_code=400,
        )
...
def denormalize_config(node: ConfigNode) -> Any:
    if node["kind"] == "object":
        return {child["key"]: denormalize_config(child) for child in node["children"]}

    if node["kind"] == "array":
        return [denormalize_config(child) for child in node["children"]]
